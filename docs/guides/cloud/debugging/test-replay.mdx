...
Cypress uses a powerful [command log](/guides/core-concepts/cypress-app#Command-Log) locally to display every command executed in your spec. The Reporter UI, including the Cypress Command Log, is rendered during `cypress open` mode, but also ([headless](/guides/guides/command-line#Options)-ly) during `cypress run` so that the Reporter UI displays within any screenshots or videos captured, to assist in debugging CI failures.

### Problem

Rendering the Cypress Reporter UI while running tests via `cypress run` often has performance implications, resulting in a slower test runtime, especially on lower resourced machines.

### Solution

With Test Replay, Cypress regenerates the entire Reporter UI within the Cloud for viewing the run and the debugging CI failures. Additionally, displaying the Cypress Reporter UI in videos and screenshots is less valuable for users using Test Replay.

Because of this, as of Cyrpess `v13`, the Cypress Reporter UI is no longer rendered during `cypress run` for users using Test Replay and `video: false`, with an option to opt out of this behavior.

When `video: true` is set, the Cypress Reporter UI will be captured as part of that video recording. You can still keep the command log enabled for test runs and artifacts in non-Chromium browsers.
